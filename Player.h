...
class Player : public Controls, public Drawable {
  ...
  Player(Point p, int s) : current(p.x, p.y), size(s)
   ...
   void updatePlayer(std::vector<bool>& v, double dt) {
      // up
      if (v[0]) {
	 speed = Vector(speed.x, speed.y - 100); // modifies the rate of change in position
	 current = current + speed * dt;	 // updates the position
      ...
      // down
      if (v[1]) {
	 speed = Vector(speed.x, speed.y + 100); 
	 current = current + speed * dt;
      ...
      // right
      if (v[2]) {
	 speed = Vector(speed.x + 100, speed.y);
	 current = current + speed * dt;
      }
      else speed = Vector(0, 0);
	 ...
	 current = current + speed * dt;
      ...
      if (v[4]) {
	 std::shared_ptr<Projectile> new_proj = std::make_shared<Projectile> (Point(400,400),
									      Vector(50,0));
